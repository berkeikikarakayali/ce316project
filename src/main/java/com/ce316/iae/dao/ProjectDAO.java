...
public class ProjectDAO {
    ...
    public Project loadProject() throws SQLException {
        String sql = "SELECT name, configuration_id, expected_output_path, run_args, " +
                     "compile_timeout_sec, run_timeout_sec, normalization_mode " +
    ...
    public void updateProject(Project project) throws SQLException {
        String sql = "UPDATE PROJECT SET name = ?, configuration_id = ?, expected_output_path = ?, " +
                     "run_args = ?, compile_timeout_sec = ?, run_timeout_sec = ?, " +
                     "normalization_mode = ? WHERE id = 1";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            ps.setString(4, project.getRunArgs());
            ps.setInt(5, project.getCompileTimeoutSec());
            ps.setInt(6, project.getRunTimeoutSec());
            ps.setString(7, project.getNormalizationMode().name());
            ps.executeUpdate();
        }
    }
}
